...
namespace engine.framework
{
    ...
    public class Vertices : List<Vector2>
    {
        ...
        {
            AABB aabb = GetAABB();
            return new Rectangle2D((int)aabb.LowerBound.X, (int)aabb.LowerBound.Y, (int)aabb.Width, (int)aabb.Height);
        }
        ...
        public int PointInPolygon(ref Vector2 point)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                Vector2 edge = p2 - p1;
                float area = MathHelper.Area(ref p1, ref p2, ref point);
        ...
        public void Transform(ref Matrix4x4 transform)
        {
            ...
            if (Holes != null && Holes.Count > 0)
            {
                for (int i = 0; i < Holes.Count; i++)
                {
                    ...
                    {
                        temp[x] = Vector2.Transform(temp[x], transform);
                    }
        ...
        public PolygonError CheckPolygon2()
        {
            if (Count < 3 || Count > Settings.MaxPolygonVertices)
                return PolygonError.InvalidAmountOfVertices;
